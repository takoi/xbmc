  ...
  : CAddon(ext)
{
  ...
  const string labelStr = CAddonMgr::Get().GetExtValue(ext->configuration, "@label");
  if (labelStr.empty())
  {
    m_label = Name();
...
void CContextItemAddon::OnPostInstall(bool restart, bool update)
{
  if (restart)
  {
    ...
    {
      ContextAddonPtr contextItem = std::dynamic_pointer_cast<CContextItemAddon>(localAddon);
      if (contextItem)
